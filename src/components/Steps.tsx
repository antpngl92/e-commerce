...
const Steps = () => {
  ...
  return (
    <ol
      className="rounded-md bg-white lg:flex lg:rounded-none lg:border-l lg:border-r lg:border-gray-200"
    >
      {STEPS.map((step, i) => {
        const isCurrentStep = pathName.endsWith(step.url)
        const isCompletedStep = STEPS.slice(i + 1).some((step) => pathName.endsWith(step.url))
                ...
                <span className="flex-shrink-0">
                  <img
                    ...
                    className={cn("flex size-20  object-contain items-center justify-center", {
                      ...
                      'border-zinc-700': isCompletedStep
                    })}
                  />
                </span>
