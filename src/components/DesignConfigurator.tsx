...
import ImageResizeCorner from "./ImageResizeCorner"
import { AspectRatio } from "./ui/aspect-ratio"
import { ScrollArea } from "./ui/scroll-area"
import { Label } from './ui/label'
import { DesignConfiguratorProps } from "@lib/types"
...
import { BASE_PRICE } from '@lib/const'
import { useUploadThing } from '@lib/uploadthing'
import { useToast } from './ui/use-toast'
import { useMutation } from '@tanstack/react-query'
import { SaveConfigArgsType, saveConfig as _saveConfig } from '@app/configure/design/actions'
...
const DesignConfigurator = ({
  ...
  const saveConfiguration = async () => {
    try {
      ...
      const file = new File([blob], `${configId}-cropped.png`, {
        type: 'image/png'
      })
    ...
    <div className="relative mt-20 grid grid-col-1 lg:grid-cols-3 mb-20 pb-20">
      <div ref={containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
        <div className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]">
          ...
          {/* gray background around phone case */}
          <div
          ...
          <div
            className={
              cn("absolute inset-0 left-[3px] top-px right-[3px] bottom-px rounded-[32px]", `bg-${options.color.tw}`)
            }
          />
        </div>
        <Rnd
          ...
          resizeHandleComponent={{
            bottomRight: <ImageResizeCorner />,
            bottomLeft: <ImageResizeCorner />,
            topLeft: <ImageResizeCorner />,
            topRight: <ImageResizeCorner />,
          }}
      ...
      <div className="h-[37.5rem] w-full col-span-full  lg:col-span-1 flex flex-col bg-white">
        <ScrollArea className="relative flex-1 overflow-auto">
          <div area-hidden="true" className="absolute z-10 index-x-0 bottom-0 h-12 bg-gradient-to-t from-white pointer-events-none"/>
            ...
            <div className="relative mt-4 h-full flex flex-col justify-between">
              <div className='flex flex-col gap-6'>
                ...
                {[MATERIALS, FINISHES].map(({ name, options: selectableOptions }) => (
                  <RadioGroup key={name} value={options[name]} onChange={
                    (val) => {
