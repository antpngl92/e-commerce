...
import { BASE_PRICE, PRODUCT_PRICES } from '@lib/const'
...
const DesignPreview = ({ configuration }: { configuration: Configuration }) => {
  ...
  const tw = COLORS.find((item) => item.value === color)?.tw
  const { label: modelLabel } = MODELS.options.find((item) => item.value === model)
  ...
  return (
    ...
    >
      <Confetti 
        active={showConfetti} 
        config={{ elementCount: 300, spread: 200 }}
      />
      </div>
          ...
          <div className='grid grid-cols-1 gap--y-8 border-b border-gray-200 py-8 sm:grid-cols-2 sm:gap-x-6 sm:py-6 md:py-10'>
            <div>
              ...
              <ol className='mt-3 text-zinc-700 list-disc list-inside'>
                ...
                <li className=''>TPU shock absorption</li>
                <li className=''>Packaging made from recycled materials</li>
                <li className=''>5 year print warranty</li>
              </ol>
            </div>
            <div>
